...
| Partner with a Brand Designer who sets direction and taste | `brand/rules.md` and `guide_02.pdf` as the designer's artefacts; the gate reads the rules | Record one short screen capture: edit a rule, rerun, watch the gate move. No deploy. That is the working relationship in ten seconds. |
