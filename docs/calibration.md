...
**Status: partial, 10 of the 20 images the issue asks for, and one of them now
carries a designer label.** What follows is a real measurement of a real gate,
...
1. **Ten more images, and all twenty labelled by the designer.** The label has
   ...
   Method decided 2026-09-14: **generate a batch of around thirty grounds
   across seeds and both models, build a numbered contact sheet, and label the
   twenty most marginal in one sitting.** `python -m pipeline.calibrate batch`
   then `sheet`. Generated grounds are the output the gate actually exists to
   judge, so they beat hand-authored cases, which would flatter it.
2. **One disagreement written up per row.** The format above has the column;
   ...
   anybody.
3. ~~A decision on take 1.~~ **Ruled 2026-09-14: one gesture.** The mark and
   the wordmark together are one lockup, so the overlap merge stands and
   `motif.02` means "one gesture, where a mark and its wordmark are one". Take
   1 is therefore a designer-labelled on-brand image, the first of the twenty.
