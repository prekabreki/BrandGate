...
rule becomes `manual` with its error recorded. The alternative is a gate that
dies on an odd PNG, and a gate that dies gets switched off.
"""
...
def score_image(img: np.ndarray, cfg: dict | None = None,
    ...
    for rule in doc["rules"]:
        ...
        if fn is None:
            manual.append(rule["id"])
            errors.append({"rule": rule["id"], "error": f"no check named {rule['check']!r}"})
            continue
        try:
            findings.append(fn(img, rule, cfg, ctx))
        except Exception as e:
            manual.append(rule["id"])
            errors.append({"rule": rule["id"], "error": f"{type(e).__name__}: {e}",
...
def _render(r: dict) -> str:
    ...
    if r["errors"]:
        lines.append(f" ! {len(r['errors'])} check error(s), recorded as manual")
    return "\n".join(lines)
